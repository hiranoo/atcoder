...
int main(){
  ...
  vector<int> order(n);
  rep(i, 0, n-1) cin >> w[i];
  rep(i, 0, n-1) cin >> order[i]--;

  rep(i, 0, n-1){
    seg.set(i, pii(w[i], order[i]));
  }
...
void main_() {
	...
	SUB(a);
	segtree<S, op, e> seg(V<ll>(n, 0));
	REP(i, n) {
		ll val = seg.prod(0, a[i]);
		seg.set(a[i], max(seg.get(a[i]), val + w[a[i]]));
	}
	print((sum - seg.all_prod()) * 2);
}
